...
# @Kailash I'm not sure we've learned type-casting, so is there a simpler way to do this?
def perfect_square(score): 
    if pow(int(pow(score, 0.5)), 2) == score:
        # print(pow(int(pow(score, 0.5)), 2))
        return True
    return False

# Same here -- I'm casting as an int because the test doesn't pass if it's a double but unsure how else to do it...
